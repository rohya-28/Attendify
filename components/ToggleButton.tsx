...
const ToggleButton: React.FC<ToggleButtonProps> = ({ role, onToggle }) => {
  return (
    <View className="my-2 w-full">
      {/* Label */}
      <Text className="text-sm font-JakartaSemiBold mb-3">
        Role
      </Text>
      ...
      <View className="flex flex-row rounded-sm overflow-hidden border border-neutral-100">
        <TouchableOpacity
          ...
          onPress={onToggle}
        >
          <Text className={`font-JakartaSemiBold text-sm ${role === 'student' ? 'text-white' : 'text-black'}`}>
            Student
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          onPress={onToggle}
        >
          <Text className={`font-JakartaSemiBold text-sm ${role === 'admin' ? 'text-white' : 'text-black'}`}>
            Admin
          </Text>
        </TouchableOpacity>
      </View>
